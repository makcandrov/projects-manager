...
public abstract class LoginPanel {
	...
	public static JPanel getPanel(ActionListener frame) {
		...
		ResultArray users = db.getAllPerosnnes();
		...
		for (int i = 0; i < nbPersonnes; i++) {
			...
			String lineD = String.format(pLine, nomStyleD, nom.toUpperCase(), prenomStyleD, prenom, qualStyleD, qualification);
			String lineH = String.format(pLine, nomStyleH, nom.toUpperCase(), prenomStyleH, prenom, qualStyleH, qualification);
		...
		JScrollPane jsp = (new JScrollPane(loginPanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER));
	    ...
	    ImageIcon newPersIconD = new ImageIcon(new ImageIcon("img/icone_new_user.png").getImage().getScaledInstance(22, 22, Image.SCALE_SMOOTH));
	    ImageIcon newPersIconH = new ImageIcon(new ImageIcon("img/icone_new_user_hover.png").getImage().getScaledInstance(22, 22, Image.SCALE_SMOOTH));
