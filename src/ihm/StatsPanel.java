...
public class StatsPanel {
	...
	public static JPanel getPanel(MainFrame frame, int projectID, int persID) {
		...
		ImageIcon printBtnD = new ImageIcon(new ImageIcon("img/icone_print.png").getImage().getScaledInstance(26, 26, Image.SCALE_SMOOTH));
		ImageIcon printBtnH = new ImageIcon(new ImageIcon("img/icone_print_hover.png").getImage().getScaledInstance(26, 26, Image.SCALE_SMOOTH));
		...
		String currentDateS = String.format("<html><span %s>Version du %02d/%02d/%d à %02d:%02d:%02d", currentDateStyle, currentDay, currentMonth, currentYear, currentHour, currentMinute, currentSecond);
        ...
        JScrollPane p1jsp = (new JScrollPane(p1, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED));
		p1jsp.getVerticalScrollBar().setUnitIncrement(4);
		p1scroll.add(p1jsp);
		
		JScrollPane p2jsp = (new JScrollPane(p2, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED));
		p2jsp.getVerticalScrollBar().setUnitIncrement(4);
		p2scroll.add(p2jsp);
		
		tabPanel.setBorder(BorderFactory.createEmptyBorder(15, 0, 0, 0));
		tabPanel.add("<html><span style='font-size:11px; font-weight:normal; font-family:Optima'>Statistiques par mission</span></html>", p1scroll);
		tabPanel.add("<html><span style='font-size:11px; font-weight:normal; font-family:Optima'>Statistiques par mois</span></html>", p2scroll);
	...
	private static JPanel getStatsByMissionPanel(ResultArray participants) {
        ...
        for (int j = 0; j < participants.size; j++) {
        	...
        	String headerS = String.format("<html><span %s> %s %s </span></html>", headerStyle, nom.toUpperCase(), prenom);
        	...
        	String bottomS = String.format("<html><span %s> %d heures </span></html>", bottomStyle, myTotalWorkTime / 3600);
	...
	public static JPanel getStatsByMissionColumn(ResultArray statRes) {
        ...
        for (int i = 0; i < statRes.size; i++) {
        	...
        	String statMissionS = String.format("<html><div %s> %s <br> %s heures</div></html>", statMissionStyle, statMission, statMissionHours);
        	...
        	String topTitledBorderS = String.format("<html><span %s> %s </span></html>", titledBorderStyle, statMissionDebutS);
        	String bottomTitledBorderS = String.format("<html><span %s>  %s</span></html>", titledBorderStyle, statMissionFinS);
        	...
        	if (!missionColors.containsKey(statMission)) {
        		...
        		missionColors.put(statMission, new Color(ran.nextInt(200)+56, ran.nextInt(200)+56, ran.nextInt(200)+56 ));
        	}
	...
	private static JPanel getStatsByMonthPanel(ResultArray participants) {
        ...
        for (int j = 0; j < timestampAL.size() - 1; j++) {
        	...
        	String monthS = String.format("<html><span %s>%s %d</span></html>", monthStyle, monthNames[calendar.get(Calendar.MONTH)], calendar.get(Calendar.YEAR));
        	
        	JLabel monthLabel = new JLabel(monthS);
        	monthLabel.setBorder(new CompoundBorder(BorderFactory.createMatteBorder(0, 1, 1, 1, Color.BLACK), BorderFactory.createEmptyBorder(7, 12, 7, 12)));
        ...
        for (int j = 0; j < participants.size; j++) {
            ...
            headerLabel.setBorder(new CompoundBorder(BorderFactory.createMatteBorder(1, 0, 1, 1, Color.BLACK), BorderFactory.createEmptyBorder(7, 7, 7, 7)));
            ...
            for (int monthIndex = 0; monthIndex < nbMonth; monthIndex++) {
            	...
            	ResultArray myMonthStats = db.getMyMonthStatsGroupByMission(projectID, participantID, begMonth, endMonth);
            	...
            	monthHoursLabel.setBorder(new CompoundBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, Color.BLACK), BorderFactory.createEmptyBorder()));
                ...
                for (int missionIndex = 0; missionIndex < myMonthStats.size; missionIndex++) {
                	...
                	String missionS = String.format("<html><span>%s - %d heures", missionValue, missionWorkTime.longValue() / 3600);
