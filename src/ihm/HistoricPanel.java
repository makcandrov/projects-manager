...
public class HistoricPanel {
	...
	public static JPanel getPanel(MainFrame frame, int projectID, int persID, int sessionIDtoEdit) {
		...
		ImageIcon goBackIconD = new ImageIcon(new ImageIcon("img/icone_back.png").getImage().getScaledInstance(26, 26, Image.SCALE_SMOOTH));
		ImageIcon goBackIconH = new ImageIcon(new ImageIcon("img/icone_back_hover.png").getImage().getScaledInstance(26, 26, Image.SCALE_SMOOTH));
		...
		String titleProjectS = String.format("<html><span style='display:inline-block'><span %s> %s •</span><span %s> %s </span></span></html>", nomProjetStyle, nomProjet.toUpperCase(), clientProjetStyle, clientProjet);
		...
		for (int i = 0; i < sessions.size; i++) {
			...
			String dateEntry = String.format("%02d/%02d/%d", calendar.get(Calendar.DAY_OF_MONTH), calendar.get(Calendar.MONTH)+1, calendar.get(Calendar.YEAR));
			
			calendar.setTimeInMillis(dateTimestamp.longValue() * 1000);
			int heure = calendar.get(Calendar.HOUR_OF_DAY);
			int minute = calendar.get(Calendar.MINUTE);
			String heureEntry = String.format("%02d:%02d", calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE));
			...
			if (missionEntry.equals("_NEW_") || missionEntry.equals("_END_")) {
				...
				for (int k = 0; k < 3; k++) {
					...
					dateCBChoices[k] = String.format("%02d / %02d / %d",calendar.get(Calendar.DAY_OF_MONTH), calendar.get(Calendar.MONTH)+1, calendar.get(Calendar.YEAR));
				}
				...
				missionJTF.setBorder(BorderFactory.createCompoundBorder(missionJTF.getBorder(), BorderFactory.createEmptyBorder(0, 5, 0, 0)));
				missionJTF.setVisible(false);
				...
				ImageIcon validerEditIcon = new ImageIcon(new ImageIcon("img/icone_valider_edit.png").getImage().getScaledInstance(22, 22, Image.SCALE_SMOOTH));
				JRoundedButton validerEditBtn = new JRoundedButton();
				validerEditBtn.setMultipleInnerColor(validerEditBtn.getDefaultInnerColor(), new Color(71, 255, 134), new Color(19, 203, 82));
				validerEditBtn.setMultipleBorderColor(validerEditBtn.getDefaultBorderColor(), new Color(19, 203, 82), new Color(71, 255, 134));
				...
				ImageIcon annulerEditIcon = new ImageIcon(new ImageIcon("img/icone_annuler_edit.png").getImage().getScaledInstance(22, 22, Image.SCALE_SMOOTH));
				JRoundedButton annulerEditBtn = new JRoundedButton();
				annulerEditBtn.setMultipleInnerColor(annulerEditBtn.getDefaultInnerColor(), new Color(255, 89, 89), new Color(222, 38, 38));
				annulerEditBtn.setMultipleBorderColor(annulerEditBtn.getDefaultBorderColor(), new Color(222, 38, 38), new Color(255, 89, 89));
				...
				String dateCelluleS = String.format("<html><span %s>Le %s à %s</span></html>", dateCelluleStyle, dateEntry, heureEntry);
				
				String missionCelluleStyle = "style='font-weight:bold; font-size:11px'";
				String missoinCelluleS = String.format("<html><span %s> %s </span></html>", missionCelluleStyle, missionEntry);
				
				String dureeCelluleStyle = "style='font-weight:normal; font-size:11px'";
				String dureeCelluleS = String.format("<html><span %s> %s </span></html>", dureeCelluleStyle, dureeEntry);
				...
				ImageIcon editIcon = new ImageIcon(new ImageIcon("img/icone_edit.png").getImage().getScaledInstance(22, 22, Image.SCALE_SMOOTH));
				JRoundedButton editBtn = new JRoundedButton();
				editBtn.setMultipleInnerColor(editBtn.getDefaultInnerColor(), new Color(255, 201, 57), new Color(251, 183, 0));
				editBtn.setMultipleBorderColor(editBtn.getDefaultBorderColor(), new Color(251, 183, 0), new Color(255, 201, 57));
				...
				ImageIcon deleteIcon = new ImageIcon(new ImageIcon("img/icone_delete.png").getImage().getScaledInstance(22, 22, Image.SCALE_SMOOTH));
				JRoundedButton deleteBtn = new JRoundedButton();
				deleteBtn.setMultipleInnerColor(editBtn.getDefaultInnerColor(), new Color(255, 89, 89), new Color(222, 38, 38));
				deleteBtn.setMultipleBorderColor(editBtn.getDefaultBorderColor(), new Color(222, 38, 38), new Color(255, 89, 89));
		...
		JScrollPane jsp = (new JScrollPane(tabPanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED));
