...
public class MainFrame extends JFrame implements ActionListener {
	...
	public MainFrame() {
		...
		try {
			...
			this.persID = db.getPersIDyIP(userIP);
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (StringOperations.regexMatching(btnName, "^pers\\d+$")) {
			...
			if (nom.length() > 0 && prenom.length() >0) {
				...
				if (db.issetPerson(nom, prenom)) {
					String erreurS = String.format("<html><span %s>Cette personne existe déjà.</span></html>", erreurStyle);
					...
					db.addPersonne(nom, prenom, poste, ipAddress, Calendar.getInstance().getTimeInMillis() / 1000);
					
					persID = db.getPersonneID(nom, prenom, poste);
			...
			if (dureeValide && missionValide && dateValide) {
				...
				calendar.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH), Integer.parseInt(dateHeureS), Integer.parseInt(dateMinuteS));
			...
			if (nbWaitingParticipants == nbParticipants - 1) {
				...
				JPanel confirmationPanel = ConfirmationPanel.getPanel(confirmation, String.format("Vous êtes le dernier membre à participer à ce projet. Si vous le terminez, il sera archivé."));
			...
			if (db.getParticipants(projectID).size <= 1) {
				...
				JPanel confirmationPanel = ConfirmationPanel.getPanel(confirmation, String.format("Vous êtes le seul membre de ce projet. Si vous quittez ce projet, il sera supprimé."));
				confirmation.add(confirmationPanel);
				confirmation.pack();
				confirmation.setVisible(true);
			...
			JPanel confirmationPanel = ConfirmationPanel.getPanel(confirmation, String.format("Tous les membres de ce projet ont indiqué qu'ils l'ont terminé. Voulez-vous le reprendre ?"));
			confirmation.add(confirmationPanel);
			confirmation.pack();
			confirmation.setVisible(true);
			...
			JPanel confirmationPanel = ConfirmationPanel.getPanel(confirmation, String.format("Voulez-vous vraiment supprimer cette session du %s ?", dateS));
			confirmation.add(confirmationPanel);
			confirmation.pack();
			confirmation.setVisible(true);
			...
			if (heureS.length() == 0 || minuteS.length() == 0) {
				valideHeure = false;
				HistoricPanel.erreurLabel.setText(String.format("<html><span %s>Heure invalide.</span></html>", erreurStyle));
				HistoricPanel.erreurLabel.setVisible(true);
			}
			if (missionS.length() == 0 || missionS.equals("_NEW_")) {
				valideMission = false;
				HistoricPanel.erreurLabel.setText(String.format("<html><span %s>Mission invalide.</span></html>", erreurStyle));
				HistoricPanel.erreurLabel.setVisible(true);
			}
			if (heuresS.length() == 0 || minutesS.length() == 0) {
				HistoricPanel.erreurLabel.setText(String.format("<html><span %s>Durée invalide.</span></html>", erreurStyle));
				HistoricPanel.erreurLabel.setVisible(true);
			}
